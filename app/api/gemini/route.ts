...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, actionType, conversationHistory = [], projectContext = [] } = await request.json()
...
[Your actual response here...]`
    ...
    if (projectContext.length > 0) {
      ...
      projectContext.slice(-10).forEach((msg: ConversationMessage) => { // Keep last 10 project messages
    ...
    if (conversationHistory.length > 0) {
      ...
      conversationHistory.slice(-6).forEach((msg: ConversationMessage, index: number) => { // Keep last 6 messages for context
    ...
    if (error instanceof Error && error.message.includes('API key')) {
      return NextResponse.json({
        content: "🔑 **API Key Configuration Error**\n\nPlease check that your Gemini API key is correctly configured.\n\n**Steps to fix:**\n\n1. Add `GEMINI_API_KEY` to your environment variables\n2. Restart your application\n\nGet your API key from [Google AI Studio](https://ai.google.dev/).",
        type: "api_key_error",
        data: {
          title: "API Key Missing",
          description: "Gemini API key not configured",
